...
Scan2::Scan2() {
  ...
  //订阅对应对topic　接收到这个topic　系统就开始进行最小二乘对解算
  calib_flag_sub_ = node_.subscrible("calib_flag", 5, &Scan2::CalibFlagCallBack, this);
...
void Scan2::scanCallBack(const sensor_msgs::LaserScan::ConstPtr &_laserScanMsg) {
  ...
  Eigen::Vector3d d_point_scan;       //激光的scanmath计算的dpose
  Eigen::VectorXd transform_matrix(3, 3); //临时变量
  ...
  //两帧scan计算本身累计的位姿　for laser_path visualization
...
void Scan2::LaserScanToLDP(sensor_msgs::LaserScan *psan, LDP& ldp) {
...
//求两帧之间的icp位姿匹配
Eigen::Vector3d  Scan2::PIICPBetweenTwoFrames(LDP& currentLDPScan, Eigen::Vector3d tmprPose) {
...
int main(int argc, char** argv) {
  ...
  odom_calib.SetDataLen(12000);
  odom_calib.SetDataZero();
  ros::spin();
